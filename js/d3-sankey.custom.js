...
var sankey = function() {
  var x0 = 0, y0 = 0, x1 = 1, y1 = 1, // extent
      // CHANGE: refactor to use a function
      dx = function() { return 24; }, // nodeWidth
      py = 8, // nodePadding
      id = defaultId,
      align = justify,
      nodes = defaultNodes,
  ...
  function sankey() {
    // CHANGE: correctly read in the sort parameter/argument
    var graph = {nodes: nodes.apply(null, arguments), links: links.apply(null, arguments), sort: sort.apply(null, arguments)};
  ...
  function computeNodeDepths(graph) {
    ...
    // CHANGE: refactor to use a node depending width
    // var kx = (x1 - x0 - dx) / (x - 1);
    graph.nodes.forEach(function(node) {
      node.x1 = (node.x0 = x0 + Math.max(0, Math.min(x - 1, Math.floor(align.call(null, node, x)))) * ((x1 - x0 - dx(node)) / (x - 1))) + dx(node);
    });
  }
  ...
  function computeNodeBreadths(graph) {
    ...
    function resolveCollisions() {
      // CHANGE: use additional parameter for the current column
      columns.forEach(function(nodes, column) {
        var node,
            dy,
            y = y0,
            n = nodes.length,
        ...
        // CHANGE: delegate the ordering of the nodes for each column
        
        // Push any overlapping nodes down.
        nodes.sort(!!graph.sort ? graph.sort(column, ascendingBreadth) : ascendingBreadth);
