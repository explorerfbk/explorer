...
if os.path.exists(dbfile):
    os.remove(dbfile)
xlsxfile = "bilancio_consuntivo.xlsx"
...
#       sql_expenses="select conti,consuntivo from bilancio where anno=%s and class_conti='Costi' and cdc='%s'" % (str(year),cdc)
        sql_incoming="select conti,consuntivo from bilancio where anno=%s and consuntivo <= 0 and cdc='%s'" % (str(year),cdc)
        sql_expenses="select conti,consuntivo from bilancio where anno=%s and consuntivo > 0 and cdc='%s'" % (str(year),cdc)
